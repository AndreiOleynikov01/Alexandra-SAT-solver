...
namespace Graph
{
	...
	{
		if (pulse == Graph::UnitPulse(CONFLICT, 0))
		{
			return &pulse;
		}
		if (pulse.type == PulseType::UnitPulse || (pulse.type == PulseType::Pulse) && pulse.isNegative())
		{
			if (pulse != *this)
			{
		...
		else
		{
			...
			{
				right_buffer.push_back(pulse.getVariables());
				for (IPulse* p : pulse.getvalues())
				{
					if (*p != *this)
					{
			...
			for (std::vector<IUnit> vec : exclusionSet)
			{
				left_buffer.push_back(vec);
			}
			...
			for (IPulse* p : definitiveSet)
			{
				result->definitiveSet.push_back(p);
				result->size++;
			}
			for (IUnit u : right_padding)
			{
				result->definitiveSet.push_back(new Graph::UnitPulse(u.value, u.variable));
				result->size++;
			}

			long int total_weight = 0;
			for (std::vector<IUnit> vec : result->exclusionSet)
			{
				long int weight = 1;
				for (IUnit u : vec)
				{
			...
			if (total_weight == pow(2, result->size))
			{
				delete result;
